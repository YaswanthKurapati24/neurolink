...
export class ModelConfigurationManager {
  ...
  private loadDefaultConfigurations(): void {
    ...
    this.applyProviderConfigurations(defaultConfigs);
  }
  ...
  private createAllProviderConfigs(): Record<string, ProviderConfig> {
    return {
      ...
      "google-vertex": this.createGoogleVertexConfig(),
      openai: this.createOpenAiConfig(),
      anthropic: this.createAnthropicConfig(),
      vertex: this.createVertexConfig(),
  ...
  /**
   * Apply provider configurations to the manager
   */
  private applyProviderConfigurations(
  ...
  /**
   * Create Google Vertex provider configuration
   */
  private createGoogleVertexConfig(): ProviderConfig {
    return {
  ...
  /**
   * Create Vertex provider configuration
   */
  ...
  private createOllamaConfig(): ProviderConfig {
    return {
      ...
      modelBehavior: {
        ...
        toolCapableModels: this.createOllamaToolCapableModels(),
      },
    };
  }

  /**
   * Create Ollama tool-capable models configuration
   */
  private createOllamaToolCapableModels(): string[] {
    return this.getConfigArray("OLLAMA_TOOL_CAPABLE_MODELS", [
  ...
  loadConfigurationsFromFile(configPath: string): void {
    try {
      ...
      if (fileExtension === ".json") {
        ...
        configData = this.parseYamlConfig(configContent);
      } else {
        throw new Error(
   ...
   */
  private parseYamlConfig(configContent: string): unknown {
  ...
  /**
   * Convert simple YAML to JSON for basic cases
   */
  private convertSimpleYamlToJson(
    ...
    for (const line of yamlLines) {
      ...
      if (this.isValidYamlLine(trimmedLine)) {
        this.parseYamlLine(trimmedLine, jsonObj);
      }
    }
  ...
  /**
   * Check if YAML line is valid for parsing
   */
  private isValidYamlLine(line: string): boolean {
    return Boolean(line) && !line.startsWith("#") && line.indexOf(":") > 0;
  }

  /**
   * Parse a single YAML line into JSON object
   */
  private parseYamlLine(line: string, jsonObj: Record<string, unknown>): void {
    const colonIndex = line.indexOf(":");
    const key = line.substring(0, colonIndex).trim();
    const value = line.substring(colonIndex + 1).trim();

    jsonObj[key] = this.convertYamlValue(value);
  }
